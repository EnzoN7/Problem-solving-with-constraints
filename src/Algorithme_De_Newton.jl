...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    if options == []
        ...
        while flag == -1 && enter
            ...
            # Resolution du systeme : hessf(x_k) d_k = - gradf(x_k)
            xpre = xmin
            d = -hessf(xpre) \ gradf(xpre)
            xmin += d
            
            # Criteres de convergence
            if norm(gradf(xmin), 2) <= max(Tol_rel * normGradfx0, Tol_abs)
                flag = 0
            elseif norm(d, 2) <= max(Tol_rel * norm(xpre, 2), Tol_abs)
                flag = 1
            elseif abs(f(xmin) - f(xpre)) <= max(Tol_rel * abs(f(xpre)), Tol_abs)
                flag = 2
